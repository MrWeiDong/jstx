            ...
            if (groupLength > 0) {
                for (var i = 0; i < groupLength; i++) {
                    ...
                    if (alias) {
						...
						console.log(revertAlias);
						
                        pattern = placeholder.replace(new RegExp('{alias}', 'gi'), '([\\s\\S]+?)');
                        regexp = new RegExp(pattern, 'gm');
						console.log(pattern);
						console.log(regexp);
                        that.$content.html(contentstr.replace(regexp, function ($0, $1) {
                            var n = revertAlias[$1];
							console.log($1);
                            if (n) {
                                return '<img class="emoji_icon" src="' + path + n + file + '"/>';
...
function stremjio(str){
	...
	for (var i = 0; i < groupLength; i++) {
		...
		if (alias) {
			...
			pattern = placeholder.replace(new RegExp('{alias}', 'gi'), '([\\s\\S]+?)');
			regexp = new RegExp(pattern, 'gm');
			htmlstr = str.replace(regexp, function ($0, $1) {
				var n = revertAlias[$1];
				if (n) {
					return '<img class="emoji_icon" src="' + path + n + file + '"/>';
				} else {
			...
			pattern = placeholder.replace(new RegExp('{alias}', 'gi'), '(\\d+?)');
			htmlstr = htmlstr.replace(new RegExp(pattern, 'gm'), '<img class="emoji_icon" src="' + path + '$1' + file + '"/>')
		}
	}
	return htmlstr;
}
